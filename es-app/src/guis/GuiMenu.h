...
private:
    ...
    static void openExternalMounts(Window* mWindow, std::string configName);
		...
		static std::shared_ptr<OptionListComponent<std::string>> getEmuVideo(Window* mWindow,
			GuiSettings* guiSettings, std::string configName);
		static std::string getVideoMode(std::string configName);
#endif
